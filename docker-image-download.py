...
if len(sys.argv) < 2:
	print('Usage:\n\tdocker_pull.py [registry/][repository/]image[:tag|@digest] [plantform]\n')
	exit(1)
...
if len(sys.argv) == 3:
	...
	# 校验machine_arch是否在合法输入中
	if machine_arch not in ['amd64', 'arm64', 'x86_64', 'x86']:
		print('Invalid machine_arch, please choose from amd64, arm64, x86_64, x86')
		exit(1)

if machine_arch in ['amd64', 'x86_64']:
	machine_arch = 'amd64'
elif machine_arch in ['arm64', 'aarch64']:
	machine_arch = 'arm64v8'
...
if 'layers' not in resp.json().keys():

	machine_arch_img = resp.json()['manifests']
	# 获取指定平台的mainfest
	for arch in machine_arch_img:
		if arch['platform']['architecture'] == machine_arch:
			auth_head['Accept'] = arch['mediaType']
			resp = requests.get('https://{}/v2/{}/manifests/{}'.format(registry, repository, arch['digest']), headers=auth_head, verify=False, proxies=proxies)
			break
	if (resp.status_code != 200):
		print('[-] Cannot fetch manifest for {} [HTTP {}]'.format(repository, resp.status_code))
		print(resp.content)
